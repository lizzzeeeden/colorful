...
public class LimitedLayout : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        cameraHeight = myCamera.GetComponent<Camera>().orthographicSize * 2;
        //初始化子弹数量
    ...
    private void ArrangeLayout()
    {
        ...
        for (int i = 1; i < bullets.Count; i++) {
            if (i % 2 != 0) {
                ...
                Vector3 pz = new Vector3(-Mathf.Sin(intervalAngle) * radius
                    , iniPosition.y - (radius - Mathf.Cos(intervalAngle) * radius)
                    , 0f);
